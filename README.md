...
<p align="center">
  <em>A GitHub repository template for ROS projects with Docker, pre-commit hooks, and CI/CD</em>
</p>
...
A GitHub repository template for ROS (Noetic) projects. Uses Docker for easy deployment and testing, with pre-commit hooks and GitHub Actions CI/CD included. The template package has both C++ and Python entrypoints.
...
1. Rename folders and source files containing **new_package** or **NewPackage**:
   ...
   - Python library directory under `src/` → `underscore_naming_convention`
   - C++ library directory under `include/` → `UpperCamelCase`

   </details>

1. Update names of **launch files** and their content; modify [launch.sh](scripts/launch.sh) accordingly — this is the entry point to the ROS node from outside the container

1. Update names of **msg/action/srv files** and reflect the changes in the corresponding `CMakeLists.txt`, C++, and Python source files
...
1. Update **ROS_MASTER_URI** and **ROS_IP** in `.env`
